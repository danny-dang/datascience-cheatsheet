...
$$
cost = \cfrac{1}{2}\sum_{i=1}^m(\hat y^{(i)} - y^{(i)} )^2 
$$
...
$$
\cfrac{\partial cost}{\partial \theta_{1}} = (\cfrac{1}{2}\sum_{i=1}^m((\theta_{0} + \theta_{1}x^{(i)})-y^{(i)})^2)' \\=\sum_{i=1}^m(\theta_{0} + \theta_{1}x^{(i)})-y^{(i)})\theta_{1}
$$
